...
struct Passenger {
	/*int*/ string survival;        //Survival
									//(0 = No; 1 = Yes)
	/*int*/ string pclass;          //Passenger Class
									//(1 = 1st; 2 = 2nd; 3 = 3rd)
	string name;					//Name
	string sex;						//Sex
	/*double*/ string age;			//Age
	double age_double;
	/*int*/ string sibsp;           //Number of Siblings / Spouses Aboard
	/*int*/ string parch;           //Number of Parents / Children Aboard
	string ticket;					//Ticket Number
	/*double*/ string fare;         //Passenger Fare
	string cabin;					//Cabin
...
ostream& operator<<(ostream& os, vector<Passenger>& x) {
	for (int i = 0; i < x.size(); i++) {
		//if ((x[i].survival == "1") && (x[i].pclass == "1") && (x[i].sex == "female")) {
			os << x[i].survival << ",";
			os << x[i].pclass << ",";
